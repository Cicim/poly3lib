...
impl<'rom> Memory<'rom> {
    ...
    pub fn read_byte(&self, address: u32) -> Result<u8, MemoryError> {
        match address {
            0x00000000..=0x00003FFF => self.bios.get(address as usize),
            0x02000000..=0x0203FFFF => self.wram.get(address as usize),
            0x03000000..=0x03007FFF => self.wram_chip.get(address as usize),
            0x04000000..=0x040003FE => self.io_registers.get(address as usize),
            0x05000000..=0x050003FF => self.bg_obj_palette_ram.get(address as usize),
            0x06000000..=0x06017FFF => self.vram.get(address as usize),
            0x07000000..=0x070003FF => self.oam.get(address as usize),
            0x08000000..=0x09FFFFFF => self.rom.get(address as usize),
            0x0E000000..=0x0E00FFFF => self.sram.get(address as usize),
            _ => panic!("Invalid address: {:08X}", address),
        }
    ...
    pub fn write_byte(&mut self, address: u32, byte: u8) -> Result<(), MemoryError> {
        let location = match address {
            0x00000000..=0x00003FFF => self.bios.get_mut(address as usize),
            0x02000000..=0x0203FFFF => self.wram.get_mut(address as usize),
            0x03000000..=0x03007FFF => self.wram_chip.get_mut(address as usize),
            0x04000000..=0x040003FE => self.io_registers.get_mut(address as usize),
            0x05000000..=0x050003FF => self.bg_obj_palette_ram.get_mut(address as usize),
            0x06000000..=0x06017FFF => self.vram.get_mut(address as usize),
            0x07000000..=0x070003FF => self.oam.get_mut(address as usize),
            0x08000000..=0x09FFFFFF => return Err(MemoryError::WriteToRom),
            0x0E000000..=0x0E00FFFF => self.sram.get_mut(address as usize),
            _ => panic!("Invalid address: {:08X}", address),
        };
        ...
        match location {
            ...
            None => Err(MemoryError::InvalidRange),
        }
    }
